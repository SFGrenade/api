    ...
    public class ModHooks
    {
        ...
        internal static void SaveGlobalSettings()
        {
            try
            {
                ...
                {
                    Logger.APILogger.Log("Saving for " + x.Name);
        ...
        /// <summary>
        ///     Called after a scene has fully initialized, all GameObjects are active and
        ///     components have run their Start methods. Provides the scene name and its root GameObjects.
        /// </summary>
        /// <remarks>N/A</remarks>
        public static event Action<string, GameObject[]> SceneObjectsLoadedHook;

        internal static void OnSceneObjectsLoaded(string sceneName, GameObject[] rootObjects)
        {
            if (SceneObjectsLoadedHook is null) return;

            foreach (Action<string, GameObject[]> toInvoke in SceneObjectsLoadedHook.GetInvocationList())
            {
                try { toInvoke.Invoke(sceneName, rootObjects); }
                catch (Exception ex) { Logger.APILogger.LogError(ex); }
            }
        }
        ...
        #region EnemyHandling

        /// <summary>
        ///     Called when an enemy takes damage. Allows modification of the damage amount.
        ///     Return the modified damage value.
        /// </summary>
        /// <see cref="OnEnemyTakeDamageHandler"/>
        /// <remarks>HealthManager.TakeDamage</remarks>
        public static event OnEnemyTakeDamageHandler OnEnemyTakeDamageHook;

        internal static int OnEnemyTakeDamage(HealthManager hm, int damage)
        {
            if (OnEnemyTakeDamageHook is null) return damage;

            foreach (OnEnemyTakeDamageHandler toInvoke in OnEnemyTakeDamageHook.GetInvocationList())
            {
                try { damage = toInvoke.Invoke(hm, damage); }
                catch (Exception ex) { Logger.APILogger.LogError(ex); }
            }
        ...
        /// <summary>
        ///     Called once when an enemy dies, guaranteed to fire exactly once per death regardless of
        ///     how many death events the enemy receives. Provides the enemy's HealthManager for context.
        /// </summary>
        /// <remarks>HealthManager.Die</remarks>
        public static event Action<HealthManager> AfterEnemyDeathHook;

        internal static void OnAfterEnemyDeath(HealthManager hm)
        {
            if (AfterEnemyDeathHook is null) return;

            foreach (Action<HealthManager> toInvoke in AfterEnemyDeathHook.GetInvocationList())
            {
                try { toInvoke.Invoke(hm); }
                catch (Exception ex) { Logger.APILogger.LogError(ex); }
            }
        }
